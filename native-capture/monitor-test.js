...
function createWindow() {
    const win = new BrowserWindow({
        width: 1320,
        height: 820,
        title: "NoxVoice Native Monitor Capture Test",
...
<head>
    <meta charset="UTF-8">
    <title>NoxVoice Native Monitor Capture Test</title>
    <style>
...
<body>
    <div class="top">
        <div>
            <div class="title">NoxVoice Fullscreen Monitor Capture Test</div>
    ...
    <div class="grid">
        <div class="panel">
            <div class="panel-title">Native monitor capture canvas</div>
            <div class="preview">
    ...
    <script>
        ...
        const monitorIndex = ${JSON.stringify(monitorIndex)};
        ...
        function listMonitorsText() {
            try {
                const monitors = nativeCapture.listMonitors();
                return monitors.map((m) => {
                    return "#" + m.index + " " + m.name + " " + m.width + "x" + m.height + (m.primary ? " primary" : "");
                }).join(" | ");
            } catch (err) {
                return "Could not list monitors: " + err.message;
            }
        }
        ...
        function drawFrame() {
            ...
            try {
                const frame = nativeCapture.captureMonitorFrame(monitorIndex);
                ...
                if (!frame.success) {
                    ...
                    statusEl.innerHTML =
                        "<code>Monitor " + monitorIndex + "</code> | " +
